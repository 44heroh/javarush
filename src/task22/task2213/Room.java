...
public class Room {
    ...
    public void print() {
        ...
        for(SnakeSection snakeSection : sections){
            gameArea[snakeSection.getY()][snakeSection.getX()] = 1;
        }
        //Рисуем голову змеи (4 - если змея мертвая)
        gameArea[snake.getY()][snake.getX()] = snake.isAlive() ? 2 : 4;
        //Рисуем мышь
        gameArea[mouse.getY()][mouse.getX()] = 3;
        //Выводим все это на экран
        String[] symbols = {".", "x", "X", "^", "RIP"};
        for(int y = 0; y < height; y++){
            for(int x = 0; x < width; x++){
                System.out.print(symbols[gameArea[y][x]]);
            }
            System.out.println();
        }
        System.out.println();
